...
export class DishesComponent implements OnInit {
    ...
    this.basketService.getMsg().subscribe((data: any) => {
      if (data.action == 'add') {
        ...
        for (const dish of this.dishes) {
          ...
          let index = this.basketInfo.findIndex(info => info._id == dish._id)
          if (index != -1)
            this.basketInfo[index].quantity = amount
        }
        this.findCheapAndExpensive();
      }
    });
  ...
  changeView(data: any): void {
    let index = this.dishes.findIndex(dish => dish._id == data.dish._id)
    if (index != -1) {
      if (data.action == 'add') {
