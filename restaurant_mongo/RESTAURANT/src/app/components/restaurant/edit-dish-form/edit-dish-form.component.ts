...
export class EditDishFormComponent implements OnChanges {
  @Input() dish!: Dish;
  @Output() dishEdited: EventEmitter<Dish> = new EventEmitter();
  modelForm: FormGroup;
  ...
  editDishFormClick(): void {
    // this.dishesService.editDish(this.dish._id, this.modelForm.value);
    this.dishesService
      .editDish(this.dish._id, this.modelForm.value)
      .subscribe((data: any) => {
        if (data.action == 'DISH_EDITED') {
          this.clearForm();
          this.dishEdited.emit(data.dish);
        }
      });
  }
